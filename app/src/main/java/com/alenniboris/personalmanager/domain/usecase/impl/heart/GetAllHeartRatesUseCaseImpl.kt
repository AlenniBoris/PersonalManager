...
class GetAllHeartRatesUseCaseImpl @Inject constructor(
    private val heartRepository: IHeartRepository,
    private val getCurrentUserUseCase: IGetCurrentUserUseCase,
    private val dispatchers: IAppDispatchers
) : IGetAllHeartRatesUseCase {

    override suspend fun invoke(): CustomResultModelDomain<List<HeartRateModelDomain>, CommonExceptionModelDomain> =
        withContext(dispatchers.IO) {
            return@withContext getCurrentUserUseCase.userFlow.value?.let { user ->
                heartRepository.getAllHeartRates(
                    userId = user.id
                )
